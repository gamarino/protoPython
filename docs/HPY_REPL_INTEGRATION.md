...
> HPy-style extension modules cannot currently be imported (the loader is not
> registered; see [HPY_USER_GUIDE.md](HPY_USER_GUIDE.md)), so they cannot be used from
> `protopy -i` either. The REPL banner still ends with "[HPy Integrated]"; that text
...
Reloading an extension module is not supported: the loader keeps no record of opened
shared libraries for unloading.
