...
# protoPython keeps an OrderedDict's order in the inherited dict, which
# preserves insertion order (a re-inserted key goes last). CPython's version
# keeps a second structure, a linked list of weakref proxies. It cannot run
# here: weakref.proxy does not forward attributes, the _collections_abc views
# and MutableMapping.update it inherits are native stubs, and a native dict
# insert would bypass the list and leave it out of sync.
class OrderedDict(dict):
    ...
    def move_to_end(self, key, last=True):
        ...
        if last:
            ...
            return
        # Re-insert every other key after `key`: O(n), where CPython's linked
        # list makes it O(1).
        rest = [(k, dict.pop(self, k)) for k in list(dict.keys(self))]
        dict.__setitem__(self, key, value)
        for k, v in rest:
