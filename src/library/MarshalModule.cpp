...
static const proto::ProtoObject* py_marshal_loads(
    ...
    if (env) {
        env->raiseValueError(context, proto::ProtoString::fromUTF8(context, "marshal.loads not implemented in ProtoPython"));
    }
    return nullptr;
}
...
static const proto::ProtoObject* py_marshal_dumps(
    ...
    PythonEnvironment* env = PythonEnvironment::fromContext(context);
    if (env) {
        env->raiseValueError(context, proto::ProtoString::fromUTF8(context, "marshal.dumps not implemented in ProtoPython"));
    }
    return nullptr;
}
