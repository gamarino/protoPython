...
static const proto::ProtoObject* py_marshal_loads(
    ...
    if (env) {
        env->raiseValueError(context, context->fromUTF8String("marshal.loads not implemented in ProtoPython"));
    }
    return nullptr;
}
...
static const proto::ProtoObject* py_marshal_dumps(
    ...
    PythonEnvironment* env = PythonEnvironment::fromContext(context);
    if (env) {
        env->raiseValueError(context, context->fromUTF8String("marshal.dumps not implemented in ProtoPython"));
    }
    return nullptr;
}
