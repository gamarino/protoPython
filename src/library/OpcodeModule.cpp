...
const proto::ProtoObject* initialize(proto::ProtoContext* ctx) {
    ...
    mod = mod->setAttribute(ctx, proto::ProtoString::createSymbol(ctx, "__name__"), proto::ProtoString::fromUTF8(ctx, "_opcode")->asObject(ctx));
    ...
    for (int i = 0; i <= maxOp; ++i) {
        ...
        if (namesVec[i].empty()) {
            nameObj = proto::ProtoString::fromUTF8(ctx, ("<" + std::to_string(i) + ">").c_str())->asObject(ctx);
        } else {
            nameObj = proto::ProtoString::fromUTF8(ctx, namesVec[i].c_str())->asObject(ctx);
        }
